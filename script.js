...
service.onchange=()=>{
  ...
  intervillesFields.style.display=circuitsFields.style.display=airportFields.style.display="none";
...
circuit.onchange=circuitPlaces.onchange=()=>{
  if(circuit.value && circuitPlaces.value){
    ...
    circuitPrix.value=base+(circuitPlaces.value>5?(circuitPlaces.value-5)*extra:0)+" €";
  } else circuitPrix.value="";
};
...
submitBtn.onclick=()=>{
  ...
  window.open(
    "https://api.whatsapp.com/send?phone=212691059759&text="+encodeURIComponent(text),
    "_blank"
  );
